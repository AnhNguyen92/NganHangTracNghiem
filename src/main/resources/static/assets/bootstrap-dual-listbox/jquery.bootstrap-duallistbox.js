	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($) {
			...
			function refreshInfo(dualListbox) {
				...
				if (all1 === 0) {
					...
					content = formatString(
							dualListbox.settings.infoTextFiltered, [ visible1,
									all1 ]);
				}
				...
				if (all2 === 0) {
					...
					content = formatString(
							dualListbox.settings.infoTextFiltered, [ visible2,
									all2 ]);
				}
						...
						function(index, item) {
							...
							if ($item.prop('selected')) {
								...
								dualListbox.elements.select2.append($item
										.clone(true).prop('selected',
												$item.data('_selected')));
							} else {
								dualListbox.elements.select1.append($item
										.clone(true).prop('selected',
												$item.data('_selected')));
							}
						});
			...
			function filter(dualListbox, selectIndex) {
				...
				dualListbox.elements['select' + selectIndex].empty().scrollTop(
						0);
				var regex, allOptions = dualListbox.element.find('option'), options = dualListbox.element;
				...
				try {
					regex = new RegExp($
							.trim(dualListbox.elements['filterInput'
									+ selectIndex].val()), 'gi');
				} catch (e) {
				...
				options.each(function(index, item) {
					...
					if (item.text.match(regex)
							|| (dualListbox.settings.filterOnValues && $item
									.attr('value').match(regex))) {
						isFiltered = false;
						dualListbox.elements['select' + selectIndex]
								.append($item.clone(true).prop('selected',
										$item.data('_selected')));
					}
					allOptions.eq($item.data('original-index')).data(
							'filtered' + selectIndex, isFiltered);
				});
			...
			function sortOptionsByInputOrder(select) {
				...
				selectopt
						.sort(function(a, b) {
							var an = parseInt(a.getAttribute('data-sortindex')), bn = parseInt(b
									.getAttribute('data-sortindex'));
			...
			function clearSelections(dualListbox) {
				dualListbox.elements.select1.find('option').each(
						function() {
							dualListbox.element.find('option').data(
									'_selected', false);
						});
			}
			...
			BootstrapDualListbox.prototype = {
				init : function() {
					...
					this
							.setRemoveSelectedLabel(this.settings.removeSelectedLabel);
					...
					this
							.setNonSelectedListLabel(this.settings.nonSelectedListLabel);
					this
							.setHelperSelectNamePostfix(this.settings.helperSelectNamePostfix);
					this
							.setSelectOrMinimalHeight(this.settings.selectorMinimalHeight);
					...
					this
							.setEventMoveAllOverride(this.settings.eventMoveAllOverride);
					this
							.setEventRemoveOverride(this.settings.eventRemoveOverride);
					this
							.setEventRemoveAllOverride(this.settings.eventRemoveAllOverride);
				...
				setMoveOnSelect : function(value, refresh) {
					...
					if (this.settings.moveOnSelect) {
						...
						this.elements.moveButton
								.insertAfter(this.elements.moveAllButton);
						this.elements.removeButton
								.insertBefore(this.elements.removeAllButton);
					}
					if (refresh) {
						refreshSelects(this);
					}
				...
				setBtnClass : function(value, refresh) {
					...
					this.elements.removeAllButton
							.attr('class', 'btn removeall').addClass(value);
					if (refresh) {
						refreshSelects(this);
					}
					return this.element;
